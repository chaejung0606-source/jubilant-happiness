...
> 이 프로젝트를 이어받아 **개발**하실 분은 [인수인계.md](인수인계.md) 를 먼저 읽어주세요.
> 구조, 배포 방법, 실제로 겪은 함정들이 정리되어 있습니다.
...
2. **＋ 회의 폴더 추가** 로 회의 1건의 자료 폴더를 넣거나,
   **＋ 상위 폴더 추가** 로 여러 회의 폴더가 든 상위 폴더를 한 번에 넣습니다.
   (창 위쪽에 드래그&드롭도 가능)
3. **작업 시작** 을 누릅니다.
4. 완료된 줄을 더블클릭하면 만들어진 회의록이 열립니다.

회의록 파일명은 `2026-07-22 수 예산집행검토.hwpx` 형태입니다. 정산 시스템 업로드에서
거부되지 않도록 대괄호·괄호 같은 특수문자는 쓰지 않고 한글·영문·숫자·공백·하이픈만 씁니다.

회의록은 **각 회의 폴더 안에** 만들어집니다. 자료와 회의록이 한곳에 모여 있어
따로 찾아다닐 필요가 없습니다. 같은 폴더를 다시 돌리면 기존 회의록을 덮어쓰지 않고
`…-2.hwpx` 로 저장하며, 이미 만들어진 회의록은 자료로 읽지 않습니다.
...
로그인이 풀리면 오른쪽 위 **`Claude 로그인`** 단추를 누르세요. 프로그램이 claude 설치
위치를 직접 찾아 로그인 창을 열어 줍니다. (`claude` 가 PATH 에 없어도 됩니다.)

처음 한 번 이메일 주소를 물어보고 `설정.txt` 에 저장합니다. 다음부터는 로그인 화면에
그 주소가 미리 채워집니다. 이 파일은 **이 PC 에만** 있고 저장소에 올라가지 않습니다.
바꾸시려면 `설정.txt` 의 `이메일 = ` 줄을 고치거나 파일을 지우면 다시 물어봅니다.
