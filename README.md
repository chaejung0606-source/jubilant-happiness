...
프로그램을 켜면 새 버전이 있는지 확인하고, 있으면 물어봅니다. **예**를 누르면
알아서 내려받아 교체한 뒤 다시 시작합니다. 다시 다운로드하실 필요가 없습니다.

- 내려받은 파일은 해시를 검증한 뒤에만 교체합니다. 검증에 실패하면 기존 버전을 유지합니다.
- 이전 버전은 `회의록도구.exe.old` 로 잠시 남았다가 다음 실행 때 지워집니다.
- 인터넷이 안 되면 조용히 넘어가고 평소대로 동작합니다.
