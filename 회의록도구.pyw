...
- 보완요청(supplement): (1) 서류 간 회의 일자·시간이 서로 다르면 어떤 서류가 어떻게 다른지, (2) 영수증 결제 시각이 회의 시작보다 이르거나 회의 종료 1시간 이내이면 그 사실을, (3) 금액이 500,000원 이상인데 참석 서명부가 없으면 그 사실을 적는다. 문제가 하나도 없으면 정확히 '보완사항 없음'이라고 적는다. (이 값은 회의록 문서 맨 위에도 표시된다.)
